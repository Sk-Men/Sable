...
export function SwipeableOverlayWrapper({
    ...
    const bind = useDrag(({ active, movement: [mx], velocity: [vx], direction: [dx], event, event: e }) => {
        ...
        if (active) {
            ...
            const swipedLeft = direction === 'left' && (val < -swipeThreshold || (vx > velocityThreshold && dx < 0));
            const swipedRight = direction === 'right' && (val > swipeThreshold || (vx > velocityThreshold && dx > 0));
